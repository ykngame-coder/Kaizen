...
describe('computeSleepScore2', () => {
  ...
  it('looks back 1 month for sleep debt — nights outside the 7-day regularity window still count', () => {
    // Regularity uses windowDays (default 7); debt has its own fixed
    // 30-day window and should still pick these up.
    const metrics: HealthMetric[] = [sleep(6, 10), sleep(6, 15), sleep(6, 20)];
    const s = computeSleepScore2(metrics, ASOF);
    const debtComponent = s.components.find((c) => c.key === 'debt');
    expect(debtComponent?.value).not.toBeNull();
    expect(debtComponent?.detail).toContain('dernier mois');
    expect(debtComponent?.detail).toContain('3 nuits');
  });

  it('needs a sustained ~1 missed night/week over the month to zero the debt score (same severity bar as the old 7-day cap, extended proportionally)', () => {
    const metrics: HealthMetric[] = Array.from({ length: 5 }, (_, i) => sleep(0, i * 7));
    const s = computeSleepScore2(metrics, ASOF);
    expect(s.components.find((c) => c.key === 'debt')?.value).toBe(0);
  });

  it('does not over-penalize a couple of short nights within the 1-month window', () => {
    const metrics: HealthMetric[] = [sleep(8, 5), sleep(7, 10), sleep(6, 15)];
    const s = computeSleepScore2(metrics, ASOF);
