...
/** Bibliothèque d'exercices — browse and search the curated catalogue with filters. */
export function ExerciseLibraryScreen(): React.JSX.Element {
  ...
  return (
    <Screen scroll>
      ...
      <Text variant="caption" color="textSubtle">{results.length} résultat{results.length > 1 ? 's' : ''}</Text>
      ...
      {results.length === 0 ? (
        ...
        <Card>
          {results.map((e, i) => (
            <View key={e.id} style={{ flexDirection: 'row', alignItems: 'center', gap: spacing[3], paddingVertical: spacing[3], borderBottomWidth: i < results.length - 1 ? 1 : 0, borderBottomColor: colors.border }}>
              ...
              <View style={{ borderRadius: radii.full, paddingHorizontal: 10, paddingVertical: 4, backgroundColor: colors.surfaceElevated }}>
                <Text variant="caption" color="textMuted">{CATEGORY_ICON[e.category]}</Text>
              </View>
            </View>
          ))}
        </Card>
