...
export function WeeklyReportScreen(): React.JSX.Element {
  ...
  const FR_MONTHS = t('sport.progress.weeklyReport.monthsShort', { returnObjects: true }) as string[];
  const D_SHORT = t('sport.progress.weeklyReport.weekdaysMin', { returnObjects: true }) as string[];
  const shortDate = (d: Date): string => `${d.getDate()} ${FR_MONTHS[d.getMonth()]}`;
  function fmtDur(sec: number): string {
    const h = Math.floor(sec / 3600);
    const m = Math.round((sec % 3600) / 60);
    return h > 0 ? t('sport.progress.weeklyReport.durationHoursMinutes', { hours: h, minutes: String(m).padStart(2, '0') }) : t('sport.progress.weeklyReport.durationMinutes', { minutes: m });
  }

  const r = useMemo(() => {
    const acts = activities.filter((a) => new Date(a.startedAt).getTime() >= since);
    const prevActs = activities.filter((a) => { const tm = new Date(a.startedAt).getTime(); return tm >= prevSince && tm < since; });
  ...
  }, [activities, health, nutrition, habits, habitLogs, now, since, prevSince, asOf, t]);
  ...
  return (
    <Screen scroll>
      <Text variant="title">{t('sport.progress.weeklyReport.title')}</Text>
      <Text variant="caption" color="textSubtle">{t('sport.progress.weeklyReport.subtitle', { from: shortDate(new Date(since)), to: shortDate(now) })}</Text>

      {/* Bilan */}
      <Card>
        <Text variant="label" color="textSubtle" style={{ letterSpacing: 1.2 }}>{t('sport.progress.weeklyReport.summaryLabel')}</Text>
        <View style={{ flexDirection: 'row', gap: spacing[5], alignItems: 'center', marginTop: spacing[3] }}>
          <ProgressRing value={r.avgRec ?? 0} size={112} thickness={11} gradient centerLabel={r.avgRec != null ? `${r.avgRec}` : '—'} caption={t('sport.progress.weeklyReport.formCaption')} />
          <View style={{ flex: 1 }}>
            {recDelta != null ? <Text variant="body" style={{ fontWeight: '700', color: recDelta >= 0 ? colors.accentData : colors.error }}>{t('sport.progress.weeklyReport.recDelta', { arrow: recDelta >= 0 ? '▲ +' : '▼ ', value: Math.abs(recDelta) })}</Text> : null}
            <Text variant="body" color="textMuted" style={{ marginTop: spacing[2], lineHeight: 21 }}>
              {r.sessions > 0 ? t('sport.progress.weeklyReport.summarySessions', { count: r.sessions, duration: fmtDur(r.totalSec) }) : t('sport.progress.weeklyReport.summaryNoSessions')}
              {r.avgRec != null ? t('sport.progress.weeklyReport.summaryRecovery', { band: recoveryBand(r.avgRec) }) : ''}
            </Text>
          </View>
        </View>
      </Card>
      ...
      {/* Chiffres clés */}
      <View>
        <SectionTitle>{t('sport.progress.weeklyReport.keyNumbersHeading')}</SectionTitle>
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: spacing[3] }}>
          <Kpi label={t('sport.progress.weeklyReport.kpi.trainings')} value={`${r.sessions}`} delta={r.sessionsDelta !== 0 ? `${r.sessionsDelta > 0 ? '▲ +' : '▼ '}${Math.abs(r.sessionsDelta)}` : t('sport.progress.weeklyReport.kpi.trainingsDeltaStable')} tone={r.sessionsDelta >= 0 ? 'up' : 'down'} />
          <Kpi label={t('sport.progress.weeklyReport.kpi.trainingTime')} value={r.totalSec > 0 ? fmtDur(r.totalSec) : '—'} />
          <Kpi label={t('sport.progress.weeklyReport.kpi.avgSleep')} value={r.avgSleep ? `${r.avgSleep.toFixed(1)} h` : '—'} delta={r.avgSleep && r.avgSleepPrev ? `${r.avgSleep >= r.avgSleepPrev ? '▲' : '▼'} ${Math.abs(Math.round((r.avgSleep - r.avgSleepPrev) * 60))} min` : undefined} tone={r.avgSleep && r.avgSleepPrev && r.avgSleep >= r.avgSleepPrev ? 'up' : 'down'} />
          <Kpi label={t('sport.progress.weeklyReport.kpi.avgRecovery')} value={r.avgRec != null ? `${r.avgRec}` : '—'} />
          <Kpi label={t('sport.progress.weeklyReport.kpi.avgHrv')} value={r.avgHrv != null ? `${r.avgHrv} ms` : '—'} />
          <Kpi label={t('sport.progress.weeklyReport.kpi.hydration')} value={r.hydration > 0 ? `${(r.hydration / 1000).toFixed(1)} L` : '—'} />
          <Kpi label={t('sport.progress.weeklyReport.kpi.caloriesBurned')} value={r.cals > 0 ? `${Math.round(r.cals)}` : '—'} />
          <Kpi label={t('sport.progress.weeklyReport.kpi.weightChange')} value={r.wDelta != null ? `${r.wDelta > 0 ? '+' : ''}${r.wDelta.toFixed(1)} kg` : '—'} />
        </View>
      </View>
      ...
      {r.trainingSeries.some((v) => v > 0) ? (
        <Card>
          <SectionTitle right={<Text variant="caption" color="textSubtle">{t('sport.progress.weeklyReport.performancesCaption')}</Text>}>{t('sport.progress.weeklyReport.performancesHeading')}</SectionTitle>
      ...
      <Card>
        <SectionTitle>{t('sport.progress.weeklyReport.recoveryHeading')}</SectionTitle>
      ...
      {r.kcalByDay.some((v) => v > 0) ? (
        <Card>
          <SectionTitle>{t('sport.progress.weeklyReport.nutritionHeading')}</SectionTitle>
      ...
      {r.activeHabits > 0 ? (
        <Card>
          <SectionTitle>{t('sport.progress.weeklyReport.habitsHeading')}</SectionTitle>
          <KV label={t('sport.progress.weeklyReport.habitsValidated')} value={`${r.validated} / ${r.target}`} />
          <KV label={t('sport.progress.weeklyReport.habitsSuccessRate')} value={r.target > 0 ? `${Math.round((r.validated / r.target) * 100)} %` : '—'} last />
      ...
      {/* Objectifs */}
      <Card>
        <SectionTitle>{t('sport.progress.weeklyReport.objectivesHeading')}</SectionTitle>
      ...
      {weekRecords.length > 0 ? (
        <Card>
          <SectionTitle>{t('sport.progress.weeklyReport.recordsHeading')}</SectionTitle>
      ...
      <View style={{ borderRadius: radii.xl, borderWidth: 1, borderColor: 'rgba(45,127,249,0.25)', backgroundColor: 'rgba(45,127,249,0.08)', padding: spacing[5] }}>
        <View style={{ flexDirection: 'row', gap: spacing[2], alignItems: 'center' }}><Icon name="brain" size={20} color={colors.info} /><Text variant="heading">{t('sport.progress.weeklyReport.analysisHeading')}</Text></View>
        <Text variant="body" color="textMuted" style={{ marginTop: spacing[2], lineHeight: 22 }}>
          {r.avgRec != null && r.avgRecPrev != null && r.avgRec >= r.avgRecPrev ? t('sport.progress.weeklyReport.analysisRecoveryImproved') : ''}
          {r.avgSleep && r.avgSleep < 7.5 ? t('sport.progress.weeklyReport.analysisSleepFocus') : r.avgSleep ? t('sport.progress.weeklyReport.analysisSleepSolid') : ''}
          {r.wDelta != null && r.wDelta <= 0 ? t('sport.progress.weeklyReport.analysisWeightGood') : ''}
        </Text>
      </View>
      ...
      <Card>
        ...
        {[
          ...
          <View key={i} style={{ flexDirection: 'row', alignItems: 'center', gap: spacing[3], paddingVertical: spacing[2] }}>
            ...
            <Text variant="body" style={{ flex: 1 }}>{p.txt}</Text>
          </View>
        ))}
      </Card>
      ...
      <View style={{ flexDirection: 'row', gap: spacing[2] }}>
        <Button label={exporting ? t('sport.progress.weeklyReport.exportPending') : t('sport.progress.weeklyReport.exportAction')} onPress={onExport} disabled={exporting || !user} />
